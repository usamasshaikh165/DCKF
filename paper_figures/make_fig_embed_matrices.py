...
np.save(str(ROOT / "results/embed_geometry/S0.npy"), S0); np.save(str(ROOT / "results/embed_geometry/S1.npy"), S1)
import json; json.dump(cats, open(str(ROOT / "results/embed_geometry/cats.json"),"w"))
...
for ax, S, title in ((axes[0], S0, "(a) Baseline CLIP"),
    ...
    ax.set_xticklabels(cats, rotation=90, fontsize=6.6)
    ax.set_yticklabels(cats if ax is axes[0] else [], fontsize=6.6)
...
fig.savefig((ROOT / "results/embed_geometry") / "fig5_heatmap_v2.pdf", bbox_inches="tight")
fig.savefig((ROOT / "results/embed_geometry") / "fig5_heatmap_v2.png", dpi=300, bbox_inches="tight")
